...
class Board(object):
    ...
    HEADER_STRING = 'BLOX'
    CURRENT_VERSION = '1'
    SUPPORTED_VERSIONS = (CURRENT_VERSION,'2')
    @staticmethod
    def read_board(file):
        """
        ...
        >>> fake_DilFile = open("Dilasha.txt", "r")
        >>> board0 = Board.read_board(fake_DilFile)
        >>> board0.goal
        (2, 3)
...
#Creating a dictionary of reverse actions
...
def next_position(pos, action, reverse_direction=False):
    ...
    >>> vert0 = ((0, 0), (0, 1))
    >>> [(action, next_position(vert0, action)) for action in ACTIONS]
    [('U', ((0, -1), (0, -1))), ('D', ((0, 2), (0, 2))), ('L', ((-1, 0), (-1, 1))), ('R', ((1, 0), (1, 1)))]

    >>> horiz0 = ((0, 0), (1, 0))
    >>> [(action, next_position(horiz0, action)) for action in ACTIONS]
    [('U', ((0, -1), (1, -1))), ('D', ((0, 1), (1, 1))), ('L', ((-1, 0), (-1, 0))), ('R', ((2, 0), (2, 0)))]

    >>> horiz1 = ((1, 1), (2, 1))
    >>> [(action, next_position(horiz1, action)) for action in ACTIONS]
    [('U', ((1, 0), (2, 0))), ('D', ((1, 2), (2, 2))), ('L', ((0, 1), (0, 1))), ('R', ((3, 1), (3, 1)))]

    >>> horiz1 = ((1, 1), (2, 1))
    >>> [(action, next_position(horiz1, action, True)) for action in ACTIONS]
    ...
    #Based on what action we receive, we will have to adjust the coordiates. We can either use a
    # switch statement or multiple if statements. We will return accordingly. There will be different
    # scenarios based on the current position of the block
  """
    rev_dict = {'U':'D',
                'D':'U',
                'R':'L',
                'L':'R'}
    #Declared direction that turns U -> D, L->R, D -> U, R -> L as an attribute of the class
    if reverse_direction == True:
      action = rev_dict[action]

    #converting tuple of tuples to a list of lists
    position = [list(x) for x in pos]

    #if the 1x2 block is standing vertically upward
    if position[0] == position[1]:
    ...
    #if the 1x2 block is horizontal 
    elif position[0] != position[1]:

      #if the 1x2 block is horizontal to the right/left
      ...
      #if the 1x2 block is horizontal up/down
    ...
    position = [tuple(x) for x in position]
